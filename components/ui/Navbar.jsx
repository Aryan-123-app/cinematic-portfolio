...
// idx matches snap position in page.js (0=video,1=hero,2=about,3..=projects,etc.)
// Calculated dynamically based on project count in profile.json
const NAV_ITEMS = [
  { label: 'Home',         idx: 0 },
  { label: 'About',        idx: 2 },
  { label: 'Projects',     idx: 3 },
  { label: 'Experience',   idx: 3 + numProjects },
  { label: 'Publications', idx: 4 + numProjects },
  { label: 'Contact',      idx: 6 + numProjects },
]
  ...
  return (
    <>
      <header ref={headerRef} className={`${styles.header} ${onIntro ? styles.introMode : ''} ${onDark ? styles.darkMode : ''}`}>
        ...
        <button
          className={`${styles.hamburger} ${menuOpen ? styles.menuOpenBtn : ''}`}
